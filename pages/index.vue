<template>
  <v-container fluid>
    <div class="intro mt-5 mb-8">
      ...
      <h2 class="mt-2">
        Just writing whatever comes to mind. <span class="emoji">😄</span>
      </h2>
    </div>
...
export default {
  ...
  methods: {
    ...
    async fetchPosts( query='' ) {
      ...
      const fetchPosts = await baseFetch
        .sortBy('createdAt', 'desc')
        .search(query)
        .skip((this.limit - 1) * (this.page - 1))
        .fetch();

      this.nextPage = fetchPosts.length === this.limit;
      const posts = this.nextPage ? fetchPosts.slice(0, -1) : fetchPosts;
